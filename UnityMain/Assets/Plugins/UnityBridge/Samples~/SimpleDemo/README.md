...
items write a JSON command into `<project>/Library/UnityBridge/in/` and log the
response from `out/`.
...
| Menu item | domain.op | What it does |
|---|---|---|
| `Ping` | `core.ping` | round-trip check; logs `pong` |
| `Print Status` | `core.status` | logs play mode, open scene, Unity version |
| `Create Cube (Roslyn cs)` | `script.cs` | compiles + runs a C# script in memory that creates `sample-cube` in the scene |

## How to use

1. Make sure the bridge is active: the editor is open with the package
   installed (a `<project>/Library/UnityBridge/status/heartbeat.json` exists).
